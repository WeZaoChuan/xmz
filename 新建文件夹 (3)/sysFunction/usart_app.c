...
const char *float_filename = "DATA/float_data.bin";
extern uint32_t dac_val_buffer[2048];
...
const char *filename = "DATA/test.bin";
...
void parse_uart_command(uint8_t *buffer, uint16_t length)
{
	...
	if (strncmp((char *)buffer , "open_dac", 7) == 0)
		{
		char *mode_str = (char *)buffer + 8;
		int mode = atoi(mode_str);
			if(mode==1) 
			{
				uart_send_flag=1;
				my_printf(&huart1, "开启dac\r\n");}
			if(mode==0)
			{
				uart_send_flag=0;
			 my_printf(&huart1, "关闭dac\r\n");}
		}
